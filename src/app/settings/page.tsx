...
const ProfileSettings = () => {
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
        ...
        toast.error(`unexpected issue: ${error.message || String(error)}`);
      }
  };
  ...
  const onSubmit = async (data: ProfileFormValues) => {
    ...
    try {
      ...
      toast.error(`Network error or unexpected issue: ${error.message || String(error)}`);
    }
  };
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          ...
          <div className="flex items-center gap-2 rounded-md border bg-background pl-4 p-2">
            ...
            {newToken && <CopyButton textToCopy={newToken} />}
          </div>
        </DialogContent>
      </Dialog>
              ...
              <table className="w-full text-sm text-left text-muted-foreground">
                ...
                <tbody>
                  {tokens.tokens.map((token) => (
                    <tr key={token.id} className="bg-background border-b hover:bg-muted/50">
